...
GPt = GaussianProcess(gamma=gamma, kernel_name="squared_exponential", d=d, loss=loss, lam=1.)
GPt.fit_gp(x2, y2)

for flo in range(2, 10):
    init_func = lambda x: torch.tensor([flo*0.1]).double()
    GPt.optimize_params(type="bandwidth", restarts=1, maxiter=1, verbose=True, optimizer='pytorch-minimize', scale=1., weight=1., init_func=init_func)
# GPt.optimize_params(type="bandwidth", restarts=10, maxiter=20, verbose=True, optimizer='pytorch-minimize', scale=1., weight=1.)

params = GPt.kernel_object.get_param_refs()
print(params)

gammas = np.linspace(0.01, 3., 300)

num_folds = min([len(x2), 20])

m = GPt._crossval_loss(GPt.kernel_object, params, num_folds=num_folds, weight=1., return_components=True)

#replace all fields with np.arrays like gammas
for k, v in m.items():
    m[k] = np.zeros_like(gammas)

m["log_marginal"] = np.zeros_like(gammas)
...
def get_marginal(i, g):
    with torch.no_grad():
        params["0"]["gamma"] = g
        m_g = GPt._crossval_loss(GPt.kernel_object, params, weight=1., num_folds=num_folds, return_components=True)
        if loss == "gaussian":
            log_marg = GPt._log_marginal_squared(GPt.kernel_object, params, weight=1.)
        else:
            log_marg = GPt._log_marginal_map(GPt.kernel_object, params, weight=1.)
        m["log_marginal"][i] = log_marg
        for k, v in m_g.items():
            m[k][i] = v
 
for i, g in enumerate(gammas):
    get_marginal(i, g)
    print(f"{i}: gamma={g}, log_m={m['log_marginal'][i]}, cv_loss={m['cv_loss'][i]}")
...
fold_keys = [k for k in m.keys() if k not in ["gamma", "cv_loss"]]
print(fold_keys)
df["q10"] = df[fold_keys].quantile(0.1, axis=1)
df["q90"] = df[fold_keys].quantile(0.9, axis=1)
df["q50"] = df[fold_keys].quantile(0.5, axis=1)
...
#csv
df.to_csv(f"investigate_cv.csv")
